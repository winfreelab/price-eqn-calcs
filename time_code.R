# PRICE EQUATION "TIME" CALCULATIONS
...
# test run loop calculates most functional years and re-runs program using those as the baseline for that site
for (test_run in c(1,0)) {

	if (test_run==1) {
		suggested_years = rep(yearlist[1], length(sitelist))
	} else {
		suggested_years = suggestions$suggested_year
	}
	...
	for (y in 1:length(yearlist)) {
		...
		names(individuals_temp) = c("site","year",splist)

		individuals_temp$site = sitelist
	...
	# set baseline site AND year here: (note: program calculates most functional years itself now)
	baseline_year = as.character(suggested_years)

	# set up a master list to put the output dataframes in from each year
	output_list = list()

	# loop through the SITES to create the baseline info for that baseline year:
	for (g in 1:length(sitelist)) {

		baseline_site = sitelist[g]
	 
		# first calculate species presence and function for the baseline year.  This will need to be done outside the loop of other years, since they all need to be compared to this information
		...
		baseline_row = subset(individuals_data, site==baseline_site & year==baseline_year[g]) 
		
		# look only at data for the baseline year
		...
		# create data frame to store total function of each year
		yearfuncs = data.frame(year=yearlist, func=rep(0,length(yearlist)))

		# loop through the COMPARISON years, calculating function, counting present species, and comparing to baseline

		for (h in 1:length(yearlist)) {
			year1 = yearlist[h]
			...
			comparison_row = subset(individuals_data, site==sitelist[g] & year==year1)
			...
			# compare each species row in the comparison year to that species row in the baseline year
			for (i in 1:nrow(comparison)) {
				# Is the species present at both years?
			...
			output_line = data.frame(comparison_year=year1, RICH_L=RICH_L, RICH_G=RICH_G, COMP_L=COMP_L, COMP_G=COMP_G, ABUN=ABUN,T_T=T_T)
			# add output from this year into the output dataframe
			output = rbind(output_line,output)
			
			yearfuncs$func[h] = comparison_total_func
				
		}

		# remove baseline year from table, since it is meaningless to compare it to itself
		output = subset(output, comparison_year!=baseline_year[g])
		...
		mostfunctionalyear = as.character(yearfuncs$year[which(yearfuncs$func==max(yearfuncs$func))])
		suggested_years[g] = mostfunctionalyear

	} # end site loop

	names(output_list) = sitelist
	
	output_list
	suggestions = data.frame(site = sitelist, suggested_year = suggested_years)
...
names(results)[1] = "site"

# write the output to csv if desired:
write.table(results, file="price time output.csv", sep=",", col.names=T, row.names=F)
#results

# we decided to get rid of site-years in which any price term returns an NA:
results2 = results
...
# Normalize results
############################
# note we are normalizing across all 5 terms and summed terms so summed terms cannot exceed 1 and can be compared in magnitude to single terms--this is different from previous versions of the code
...
price_summary = aggregate(data=output2, value ~ price_term + site, FUN=function(x) c(mean =mean(x), median=median(x)))
